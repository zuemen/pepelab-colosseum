...
- **In the app: set it up from a Base Account.** The Sessions page has a card that sends the same batch through the wallet (EIP-5792 `wallet_sendCalls`, atomic), adding SpendPermissionManager as an owner first if the account lacks it; the session's credential is then signed from the list. A test pins the calldata byte for byte to the batch that ran on chain. A browser test drives the deployed page on a Base Sepolia fork with a mock wallet that behaves like a Coinbase Smart Wallet: 20/20 checks in two scenarios, including the agent topping up margin with the exact permission the page shows and the agent's verifier accepting the credential, plus a wallet-rejection scenario, 8/8 ([`demo/BASE_ACCOUNT_UI_E2E.md`](../demo/BASE_ACCOUNT_UI_E2E.md)). Not yet tried with a real Base Account popup.
...
| FeeRouter (perp fees) | [`0x91E4aC532201Fc67C715Aa202B6Fe85F51b34994`](https://base-sepolia.blockscout.com/address/0x91E4aC532201Fc67C715Aa202B6Fe85F51b34994?tab=contract) |
...
2. **Base-native stack.** Spend Permissions for the cash leg are live on Base Sepolia, with an in-app setup from a Base Account (§5); next, a run with a real Base Account popup, Paymaster-sponsored delegation; x402 Bazaar listing; an ERC-8004 identity entry for each agent (the standard is still a Draft: [EIP-8004](https://eips.ethereum.org/EIPS/eip-8004)).
...
| **Base Account + Spend Permissions.** `SpendPermissionMarginFunder` (8 fork tests against Base Sepolia's real SpendPermissionManager and Coinbase Smart Wallet factory), deployed and authorized on Base Sepolia; agent SDK and MCP `top_up_margin`; authorization credentials signed by smart accounts (ERC-1271), accepted by every agent entry point; an end-to-end run from a Base Account; the in-app setup card on the Sessions page with a fork browser test. | `1f38197`, `c72b8e7`, `0035d44`, `b3b48bf`, `ee6c161`, `6deca42`, `8e4fdf7`, `7eea7fe`; deploy tx `0x5f6ecfa0…`, authorize tx `0x4d07412c…` |
