/**
 * 合約 revert 的說明，以及認不出來時的退路。
 *
 * `contract` 的**順序有意義**：prettyError 會照插入順序逐一做 keyword 比對，順序一
 * 變，某些錯誤就會先撞上另一條較早的規則、拿到不對的訊息。搬進 catalog 時逐字保留
 * 原檔的順序與內容。
 *
 * 合約錯誤代號（StalePrice、maxOracleAge…）刻意留在句子裡不翻——使用者要能把它貼到
 * 搜尋或回報訊息裡（見 docs/adr/0002）。
 */
