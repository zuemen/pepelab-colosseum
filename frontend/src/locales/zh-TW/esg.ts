...
export const esg = {
  /**
   * #152：見證碳等級。跟上面那組 AAA→CCC 的 ESG 評等**刻意不合併**——前者講
   * 「整體永續表現」,這個講「鏈上見證到的定價依據」,資料來源與用途都不同。
   *
   * chip 層級的字全部住在這裡,/tokens 的詳情面板也讀這一組。同一句話在兩個
   * catalog 各寫一份,遲早會漂成兩種講法。
   */
  attested: {
    title: '見證碳等級',
    lead: '等級本身就是被見證的事實——由見證者連同判定依據一起記錄在鏈上，不是讀取時從碳強度換算出來的。金庫的鑄造費率用的就是這個等級。',
    vsOld:
      '上方的 ESG 評等講的是整體永續表現（E／S／G 三軸的綜合評分）；這一區講的是鏈上見證的定價依據。兩者資料來源不同、用途也不同，所以分開呈現，不合併成單一分數。',
    column: {
      asset: '資產',
      tier: '見證等級',
    },
    countUnit: '{count} 筆見證',
    noAttestations: '尚無有效見證',
    agree: '見證一致',
    apart: '見證相差 {n} 級',
    unrated: '未評等',
    failed: '讀取見證失敗，暫時無法確認。這不代表「未評等」。',
    loading: '讀取見證中…',
  },
